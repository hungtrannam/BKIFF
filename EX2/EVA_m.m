...
m_list = [];
for IR = IR_list
    f = sprintf('EVA/YESNO_IR%d_all_m.mat', IR);
    if exist(f,'file')
        S = load(f,'m_list'); 
...
for r = 1:numel(metrics)
    ...
    for ii = 1:numel(IR_list)
        ...
        for mi = 1:Mnum
            mval = m_list(mi);
            f_one = sprintf('EVA/YESNO_IR%d_m%.3f.mat', IR, mval);
        ...
        % Vẽ chỉ đường median, fill màu cho marker theo IR
        temp(20,30,10);
    ...
    xlabel(ax,'$m$ (fuzziness)'); ylabel(ax, met);
    xlim(ax, [min(m_list)*0.95, max(m_list)*1.05]);

    if any(strcmpi(met,{'NMI','ARI'})), ylim(ax,[0 1.05]); end
